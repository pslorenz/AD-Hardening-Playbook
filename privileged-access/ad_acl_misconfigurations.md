...
When a low-privileged principal holds any of these on a sensitive object like Domain Admins, krbtgt, the domain root, a DC computer object, a service account in a privileged group, it's a direct escalation primitive.
...
Best approach: run BloodHound. It enumerates and visualizes these ACEs across the entire domain. See [`bloodhound-attack-paths.md`](bloodhound-attack-paths.md).
...
# AdminSDHolder ACL propagates to every protected object hourly
Get-DangerousAces "CN=AdminSDHolder,CN=System,$((Get-ADDomain).DistinguishedName)"
...
## Each ACL finding falls into one of three buckets:
    ...
    Obviously wrong — "Domain Users has GenericAll on Domain Admins" type stuff. Just remove it.
    Legitimate but over-scoped — a help-desk group has ResetPassword on the entire domain when it should only need it on a specific OU. Re-scope.
    Legitimate and necessary — usually scoped delegation that's been documented somewhere. Confirm with the team that requested it.
...
For inherited ACEs, the source is on a parent OU so fix it there or block inheritance with caution.
...
- Changes to AdminSDHolder are high impact and propagate quickly, lab test before just sending it. 

## Rollback (note: this rolls back the whole descriptor, so interim changes between removal and rollback will be lost)

```powershell
$dn = "<DN>"
$sddl = Get-Content "$env:TEMP\acl-backup-<...>.txt" -Raw
$acl = Get-Acl "AD:$dn"

...
- **Shadow Credentials abuse**: similar — attacker uses their ability to create accounts to set up persistence.
...
If the only creators are IT staff, service accounts, and Autopilot, you're safe to set MAQ to 0.
...
- Some self-service VDI provisioning flows (Citrix, VMware Horizon) — confirm those use a service account with explicit Create Computer rights, which is the standard config.
...
Re-run the `New-ADComputer` test as a regular user — should fail with "Access is denied."
